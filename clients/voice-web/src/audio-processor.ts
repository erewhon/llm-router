// @ts-nocheck
// Verbatim from the upstream Kyutai Moshi web client (Apache-2.0): a jitter-buffered
// AudioWorklet that plays 24 kHz PCM frames pushed from the decoder worker, growing
// its buffer on underrun and dropping old packets on overrun. The hard, tuned part
// of low-latency playback — reused as-is (see clients/voice-web/README.md).
...
class MoshiProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    if (out_idx < output.length) {
      ...
      console.log(
        "Increased partial buffer to",
        asMs(this.partialBufferSamples),
      );
