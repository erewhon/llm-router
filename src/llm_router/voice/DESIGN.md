...
- [ ] **Slice 5 — live browser frontend (chosen: orchestrator-as-Moshi-proxy).** The orchestrator becomes a persistent websocket **server** on hypatia speaking Moshi's protocol to the browser, and a Moshi *client* to real Moshi (:8998) — transparent passthrough, tapping the streams. Reuse **Moshi's existing web client** (the JS/WASM dist), just point its ws URL at the orchestrator. Steps:
